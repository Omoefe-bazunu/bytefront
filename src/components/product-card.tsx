  ...
  return (
    <Card className={cn("overflow-hidden flex flex-col h-full group", className)}>
      <CardHeader className="p-0 relative">
        ...
        {(product.isNew || product.discountedPrice) && (
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            ...
            {product.discountedPrice && <Badge variant="destructive">Sale</Badge>}
          </div>
        )}
      </CardHeader>
      ...
      <CardFooter className="p-4 pt-0 flex justify-between items-center">
        <div>
          {product.discountedPrice ? (
            <div className="flex items-baseline gap-2">
              <p className="text-xl font-bold text-primary">₦{product.discountedPrice.toLocaleString()}</p>
        ...
        <Button size="icon" variant="outline" aria-label="Add to cart" onClick={handleAddToCart}>
