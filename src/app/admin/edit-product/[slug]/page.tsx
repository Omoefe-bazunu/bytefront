...
export default function EditProductPage() {
    ...
    if (e.target.files) {
      ...
      files.forEach(file => {
          if (file.size > MAX_FILE_SIZE) {
            toast({ variant: 'destructive', title: 'Error', description: `File ${file.name} is too large.`});
            return;
          }
          if (!ACCEPTED_IMAGE_TYPES.includes(file.type)) {
            toast({ variant: 'destructive', title: 'Error', description: `File ${file.name} has an invalid type.`});
            return;
          }
      });
  ...
  const removeExistingImage = async (index: number) => {
      ...
      try {
          ...
          if (error.code !== 'storage/object-not-found') {
             toast({ variant: 'destructive', title: 'Error', description: 'Failed to delete existing image from storage.' });
             return;
          }
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (imageUrls.length + newImageFiles.length === 0) {
        toast({ variant: 'destructive', title: 'Error', description: 'Product must have at least one image.'});
        setLoading(false);
        return;
    }
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="e.g., StellarBook Pro 15" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Describe the product..." {...field} className="min-h-[150px]" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
                 ...
                 <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input type="number" placeholder="850000" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input type="number" placeholder="825000" {...field} value={field.value ?? ''} onChange={field.onChange} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                 ...
                 <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input placeholder="Stellar" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {imageUrls.map((url, index) => (
                            <div key={url} className="relative group">
                                <Image src={url} alt={`product image ${index + 1}`} width={150} height={150} className="w-full h-32 object-cover rounded-md" />
                        ...
                        {newImageFiles.map((file, index) => (
                           <div key={index} className="relative group">
                              ...
                              <Button type="button" variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => removeNewImage(index)}>
                                 <Trash2 className="h-4 w-4"/>
                              </Button>
                           </div>
                        ))}
                        <Label htmlFor="image-upload" className="cursor-pointer flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-md hover:bg-muted transition-colors">
                            <Upload className="h-8 w-8 text-muted-foreground" />
                            <span className="mt-2 text-sm text-muted-foreground">Upload</span>
                            <Input id="image-upload" type="file" className="hidden" multiple onChange={handleImageChange} accept={ACCEPTED_IMAGE_TYPES.join(",")} />
                         </Label>
                    </div>
                </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="e.g., laptop desk" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
